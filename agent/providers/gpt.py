...
class GPTProvider(AgentProvider):
    ...
    def respond(self, request: AgentRequest, trace: RequestTrace | None = None) -> AgentResponse:
        span_name = "llm.plan.generate" if request.mode == "plan" else "llm.code.generate" if request.mode == "code" else "llm.chat.generate"
        llm_span = trace.span(span_name, provider=self.name, model=self.model, mode=request.mode) if trace else None
        prompt = request.prompt
        if request.mode == "plan":
            prompt = build_plan_prompt(request.prompt, request.context)
